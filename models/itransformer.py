...
class UnivariateTransformer(nn.Module):
    ...
    def forward(
        ...
        tokens = self.embed_spikes(spikes.unsqueeze(3))         # (bs, seq_len, n_channels, hidden_size)

        if spikes_timestamp is None:
            spikes_timestamp = torch.arange(0, tokens.size(1), dtype=torch.int64, device=spikes.device)
        tokens += self.embed_pos(spikes_timestamp).unsqueeze(2)
        tokens = tokens.transpose(1,2)                          # (bs, n_channels, seq_len, hidden_size)
        tokens = torch.cat((self.cls_embed(torch.zeros_like(tokens[:,:,:1,0]).to(torch.int64)), tokens),dim=2)

        B, N, T, H = tokens.size()
        embeds = self.transformer(tokens.view(B*N,T,H)).view(B,N,T,H)
...
class iTransformerEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.mode = config.embedder.mode
        if self.mode == "mlp":
            ...
            self.embed_proj = nn.Sequential(
                nn.Linear(config.embedder.hidden_size, config.hidden_size),
        ...
        if self.embed_region:
            ...
            self.region_to_indx = {r: i for i,r in enumerate(self.regions)}
        ...
        self.embed_depth = config.embed_depth
        if self.embed_depth:
            self.depth_embeddings = nn.Sequential(
                nn.Linear(1,config.hidden_size),
                ACT2FN[config.activation],
                nn.Linear(config.hidden_size,config.hidden_size),
                nn.LayerNorm(config.hidden_size)
    ...
    def forward(
        ...
        if self.mode == "mlp":
            ...
            tokens = self.embed_proj(self.embed(spikes, spikes_timestamp))  # (batch, n_channels, hidden_size)
        
        if self.embed_channel:
            if spikes_spacestamp is None:
                spikes_spacestamp = torch.arange(0, tokens.size(1), dtype=torch.int64, device=spikes.device)
            channel_embeds = self.channel_embeddings(spikes_spacestamp) # (n_channels, hidden_size)
